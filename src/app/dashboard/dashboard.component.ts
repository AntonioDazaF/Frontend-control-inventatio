...
export class DashboardComponent implements OnInit {
  ...
  chartOptions: ChartOptions<'bar'> = {
    ...
    plugins: {
      ...
      tooltip: {
        callbacks: {
          label: (context) => {
            const valor = context.parsed && typeof context.parsed.y === 'number' ? context.parsed.y : 0;
  ...
  ngOnInit(): void {
    forkJoin({
      ...
      const totalProductos =
        resumen && typeof resumen.totalProductos === 'number'
          ? resumen.totalProductos
          : productos.length;

      const resumenMovimientos = resumen && typeof resumen.movimientos === 'number' ? resumen.movimientos : null;
      const movimientosLength = Array.isArray(movimientos) ? movimientos.length : null;
      const resumenMovimientosHoy = resumen && typeof resumen.movimientosHoy === 'number' ? resumen.movimientosHoy : null;

      const movimientosTotales =
        resumenMovimientos !== null
          ? resumenMovimientos
          : movimientosLength !== null
            ? movimientosLength
            : resumenMovimientosHoy !== null
              ? resumenMovimientosHoy
              : 0;

      const movimientosCalculadosHoy = this.contarMovimientosDeHoy(movimientos);
      const movimientosDelDia =
        typeof movimientosCalculadosHoy === 'number' && !Number.isNaN(movimientosCalculadosHoy)
          ? movimientosCalculadosHoy
          : resumenMovimientosHoy !== null
            ? resumenMovimientosHoy
            : movimientosTotales;

      const stockBajoTotal = distribution.stockBajo;
      const alertasActivas = Math.max(
        stockBajoTotal,
        this.toNumber(resumen ? (resumen as any).alertasActivas : undefined)
      );
  ...
  private contarMovimientosDeHoy(movimientos: any[] | null | undefined): number {
    ...
    return movimientos.reduce((total, movimiento) => {
      const fechaMovimiento = movimiento ? movimiento.fecha : undefined;
      return this.esMovimientoDeHoy(fechaMovimiento) ? total + 1 : total;
    }, 0);
  }
  ...
  private agruparMovimientosPorDia(movimientos: any[] | null | undefined) {
    ...
    const agrupado = movimientos.reduce<Record<string, { entradas: number; salidas: number }>>((acc, movimiento) => {
      const fechaMovimiento = movimiento ? movimiento.fecha : undefined;
      const fecha = this.parseFecha(fechaMovimiento);
      if (!fecha) {
        return acc;
      }
  ...
  private obtenerTopProductos(productos: any[]): TopProductoVista[] {
    ...
    const ordenados = [...productos].sort(
      (a, b) => this.toNumber(b && b.stock) - this.toNumber(a && a.stock)
    );
  ...
  private obtenerMovimientosRecientes(movimientos: any[] | null | undefined): MovimientoReciente[] {
    ...
    return [...movimientos]
      .map((movimiento) => {
        const fechaMovimiento = movimiento ? movimiento.fecha : undefined;
        const fecha = this.parseFecha(fechaMovimiento);
        const cantidad = this.toNumber(movimiento ? movimiento.cantidad : undefined);
        const tipo = ((movimiento && movimiento.tipo) || '').toUpperCase();
        const producto =
